...
class Unicamp_ju():
    ...
    def extract_list_editions_after_2011(self):
        ...
        for ano in range(2015, 2011, -1):
            ...
            for p in xrange(2, -1, -1):
                ...
                if r is None:
                    logger.info(u'erro ao iniciar extração do jornal da unicamp, saindo...')
                    return False
                ...
                for edicao in tree.xpath(u'//div[contains(@class, "view-content")]/h3'):
                    ...
                    self.extract_version3(edicao.text_content() + u' - ' + unicode(ano), link)
    ...
    def extract_list_editions_until_2011(self):
        ...
        try:
            ...
            for ano_div in anos:
                ano = ano_div.xpath(u'.//span[@class="ano_title"]')[0].text_content().strip()
                ...
                for tr in ano_div.xpath(u'.//tr'):
                    ...
                    if len(tds) >= 2:
                        try:
                            ...
                            links.append([ano, re.sub(ur'[ ]{2,}', u' ', link.text_content().replace(u'\n', u'').replace(u'\t', u'')), link.get(u'href')])
                        except:
                            continue
    ...
    def extract_version1(self, edition, url, tree):
        ...
        try:
            links = tree.xpath(u'//*[contains(text(), "Leia nesta edi")]/ancestor::tr[1]/following-sibling::tr')
    ...
    def extract_version1_news(self, edition, titulo, url):
        ...
        if re.search(ur'^(Cartas|Painel da semana|Teses |Painel da|Portal da|Livro da)', titulo):
            return
        ...
        subtitulo = u''
        try:
            div = tree.xpath(u'//img[contains(@src, "edit_jornal.jpg")]/ancestor::td[1]')[0]

            subtitulo = re.sub(ur'[\t\n]', u'', u''.join(div.xpath('.//i/text()')))
        except Exception as e:
            logger.exception(u'erro dentro da reportagem')

        self.tabela.append([edition, titulo, subtitulo, self.publicacao, url, u''])
        self.tabela2.append([edition, titulo + u' - ' + subtitulo, self.publicacao, url, u''])
    ...
    def extract_version2_news(self, edition, titulo, url):
        if len(titulo) == 0 or re.search(ur'Capa da Edi|Teses', titulo) != None:
            return
        ...
        subtitulo = u''
        try:
            subtitulo = limpa(tree.xpath(u'//td[@class="titulos"][1]//em')[0].text_content())
        except Exception as e:
            # print url
    ...
    def extract_version3(self, edition, url):
        ...
        for reportagem in tree.xpath(u'//div[@class="left_j_bar"]//div[@class="field-item even"]/a')[1:]:
            self.extract_version3_news(edition, limpa(reportagem.text_content()), reportagem.get(u'href'))
    ...
    def extract_version3_news(self, edition, titulo, url):
        try:
            ...
            subtitulo = tree.xpath(u'//span[@class="sub_title"]')[0].text_content()
