...
class Jornal_da_ciencia():
    ...
    def extract(self):
        ...
        for edicao in edicoes:
            ...
            edicao = tree.xpath(u'//div[@class="resumo-principal"]/strong/text()')[0].strip()

            print edicao

            noticias = tree.xpath(u'//div[@class="home-materia-edicao" or @class="materia-principal"]')

            for noticia in noticias:
                categoria = u''

                try:
                    link_c = noticia.xpath(u'.//div[@class="home-titulo-materia" or @class="titulo-principal"]//a')[0]
                    titulo = regex.sub('^[0-9]{1,}\.\s', u'', link_c.text).strip()
                    link = link_c.get(u'href').strip()

                    try:
                        subtitulo = noticia.xpath(u'.//div[@class="home-texto-materia" or @class="resumo-principal"]//p/text()')[0].strip()
                    except:
                        subtitulo = u''

                    #print edicao + u'\n' + titulo + u'\n' + subtitulo + u'\n' + link + u'\n'

                    self.tabela.append([edicao, titulo + u': ' + subtitulo, self.publicacao, link, categoria])
                    self.tabela2.append([edicao, titulo, subtitulo, self.publicacao, link, categoria])
