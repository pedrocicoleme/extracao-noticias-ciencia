...
class Unb_ciencia():
    ...
    def extract(self):
        ...
        for noticia in noticias:
            categoria = noticia.xpath(u'.//span[@class="chapeuverdecx"]/text()')[0].strip()
            data_publicacao = noticia.xpath(u'.//span[@class="datafina"]/text()')[0].split(u',', 1)[1].strip()
            ...
            subtitulo = regex.sub(ur'[\n\r\s]{1,}', u' ', noticia.xpath(u'.//div[@id="content"]/text()')[0]).strip()
            subtitulo = regex.sub(ur'^((\p{Lu}){2,}[\s\n\r]*){1,}', u'', subtitulo, regex.UNICODE)

            print data_publicacao + u'\n' + titulo + u'\n' + subtitulo + u'\n' + self.url_base + link + u'\n' + categoria

            self.tabela.append([data_publicacao, titulo + ': ' + subtitulo, self.publicacao, self.url_base + link, categoria])
            self.tabela2.append([data_publicacao, titulo, subtitulo, self.publicacao, self.url_base + link, categoria])
